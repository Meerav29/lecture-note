...
export function SummaryView({ lecture, content, onContentUpdate }: SummaryViewProps) {
    ...
    try {
      const response = await fetch('/api/generate', {
        ...
        body: JSON.stringify({
          ...
          mode: 'notes' // Using notes mode for summary
        })
      });
  ...
  if (!content && !isGenerating) {
    return (
      ...
      >
        ...
        <h3 style={{ fontSize: '1.5rem', margin: 0 }}>No Summary Yet</h3>
        <p style={{ color: 'var(--text-muted)', textAlign: 'center', maxWidth: '500px' }}>
          Generate a concise summary of your lecture with key takeaways and main points.
        </p>
  ...
  if (isGenerating) {
    return (
      ...
      >
        ...
        />
        <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>Generating summary...</p>
        <style jsx>{`
          @keyframes spin {
